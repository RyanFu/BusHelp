...
{
    [self downloadImageByImageArray:imageArray success:^(NSDictionary *dictionary){
        [self POST:SAVENEWTASK parameters:@{@"task_id" : task_id,@"org_id" : org_id,@"task_title" : task_title,@"task_content" : task_content,@"task_manager" : task_manager,@"task_begin_time" : task_begin_time,@"task_end_time" : task_end_time,@"vehicle_ids" : vehicle_ids} attachmentDic:dictionary success:^(id data){
            success(data);
        } failure:failure];
