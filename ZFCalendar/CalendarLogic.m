...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    if (dic) {
        for (int i = 1; i < daysCount + 1; ++i) {
            ...
            {
                mark=[NSString stringWithFormat:@"%i-0%i-%i",components.year,components.month,i];
            }else if(components.month<10&&i<10)
            {
                mark=[NSString stringWithFormat:@"%i-0%i-0%i",components.year,components.month,i];
            }else if (components.month>10&&i<10)
            {
                mark=[NSString stringWithFormat:@"%i-%i-0%i",components.year,components.month,i];
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    NSDateComponents *calendarToDay  = [select YMDComponents];//今天
    ...
    //没被点击选中
        ...
        if (calendarToDay.year >= calendarDay.year &
            ...
            calendarDay.style = CellDayTypeFutur;//mark
